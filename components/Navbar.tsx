...
const DesktopProfileNavbar: React.FC = (): React.ReactNode => {
...
const DesktopNavbar: React.FC<{ pathname: string }> = ({
  pathname,
}): React.ReactNode => {
  return (
    <nav className="w-[330px] hidden lg:block sticky top-0 h-full pt-2">
      <DesktopProfileNavbar />
      <ScrollArea
        className="relative px-8 py-0 mt-6 max-h-[40vh]"
        type="auto"
        scrollbars="vertical"
      >
        <ul className="space-y-2">
          {navigationList.map(({ name, icon: Icon, href }) => (
            <li key={name}>
              ...
              >
                <Link href={href}>
                  <Icon className="w-6 h-6 mr-2" /> {name}
                </Link>
              </Button>
            </li>
          ))}
...
const MobileNavbar: React.FC<{ pathname: string }> = ({
  pathname,
}): React.ReactNode => {
  const { setTheme } = useTheme();
  return (
    <nav className="fixed bottom-0 z-50 w-screen h-14 lg:hidden bg-background">
      <div className="grid h-full grid-cols-6 grid-col">
        {navigationList.map(({ name, icon: Icon, href }) => (
          <Button
            key={name}
            asChild
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={() => setTheme("system")}>
              <MonitorIcon className="w-6 h-6 mr-2" />
              System
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
